...
function handleControlRequest (message) {
	...
	const cmd = spawnSync(config.server.PATH_knx, ['raw', '0/4/' + (id_room + 1) , percent, '2', '2'])
	if(cmd.stdout.toString()){
		res_str = cmd.stdout.toString()
		console.log(`KNX server response: ${cmd.stdout.toString()}`)
	}
	const cmd2 = spawnSync(config.server.PATH_knx, ['raw', '0/4/' + (id_room + 2) , percent, '2', '2'])
...
function handleAppExit (options, err) {
  ...
  if (options.cleanup) {
    client.publish('radiator/connected', 'false')
  }
...
function intervalFunc() {
 console.log("send data radiator")

	// foreach radiator in each room
	...
	client.publish('data/radiator', data)
